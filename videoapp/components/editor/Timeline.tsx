...
export default function Timeline({
  ...
  return (
    <div style={{ height: `${height}px`, backgroundColor: '#1a1a24', borderTop: '1px solid var(--border)', display: 'flex', flexDirection: 'column' }}>
        ...
        onDrop={(e) => {
          ...
          try {
            ...
            if (data.type === 'addLayer' && data.payload) {
              e.preventDefault();
              e.stopPropagation();
              const newLayer = { ...data.payload };

              const rect = timelineRef.current?.getBoundingClientRect();
              if (rect) {
                const x = e.clientX - rect.left + timelineRef.current!.scrollLeft;
                const time = Math.max(0, (x - trackHeaderWidth) / zoom);
                newLayer.startAt = time;
              }

              delete newLayer.track; // Force new track creation
              addLayer(newLayer);
            }
          } catch (err) {
      ...
      >
        {isDraggingOver && (
          <div style={{ position: 'absolute', inset: 0, border: '2px dashed var(--accent)', backgroundColor: 'rgba(139, 92, 246, 0.05)', zIndex: 100, pointerEvents: 'none', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <div style={{ color: 'var(--accent)', fontWeight: 600, fontSize: '14px', backgroundColor: 'var(--bg-panel)', padding: '10px 20px', borderRadius: '8px', boxShadow: '0 4px 12px rgba(0,0,0,0.5)' }}>
              Drop to add to new track
            </div>
          </div>
        )}
