        ...
        onTransformEnd={(e) => {
          ...
          onChange({
            transform: {
              ...
              width: Math.max(5, Math.abs(layer.transform.width * scaleX)),
              height: Math.max(5, Math.abs(layer.transform.height * scaleY)),
              flipX: scaleX < 0,
              flipY: scaleY < 0,
            }
          });
...
const ShapeElement = ({ layer, isSelected, onSelect, onChange }: any) => {
  ...
  const commonProps = {
    ...
    onClick: onSelect,
    onTap: onSelect,
    onDragEnd: (e: any) => onChange({ transform: { ...layer.transform, x: e.target.x(), y: e.target.y() } }),
  ...
  if (layer.content === 'circle') {
    ...
    element = (
      <RegularPolygon
        ...
        width={layer.transform.width}
        height={layer.transform.height}
        // Konva RegularPolygon x/y is the center, so we offset the position
    ...
    element = (
      <Star
        ...
        width={layer.transform.width}
        height={layer.transform.height}
        // Konva Star x/y is the center, so we offset the position
        x={transitionStyle.x + layer.transform.width / 2}
        y={transitionStyle.y + layer.transform.height / 2}
        scaleX={transitionStyle.scaleX}
        scaleY={transitionStyle.scaleY}
...
export default function MainCanvas() {
  ...
  const handleDrop = (e: React.DragEvent) => {
    ...
    try {
      ...
      const canvasX = (dropX - stageRect.x) / scale;
      const canvasY = (dropY - stageRect.y) / scale;

      const newLayer = { ...data };
      // Map to center of dropped position if transforming visual elements
      if (newLayer.transform && newLayer.type !== 'audio') {
        newLayer.transform.x = canvasX - (newLayer.transform.width || 0) / 2;
        newLayer.transform.y = canvasY - (newLayer.transform.height || 0) / 2;
      }

      // Auto-assign to first visible track id or a new track if possible
      if (!newLayer.track && tracks.length > 0) {
        newLayer.track = tracks[0].id;
      }
