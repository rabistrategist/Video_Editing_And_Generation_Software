            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', marginTop: '4px' }}>
                <div>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
                    <label style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '12px', color: TEXT_MUTED }}>Rotation <RotateCw style={{ width: '16px', height: '16px' }} /></label>
                    <span style={{ fontSize: '12px', color: 'white' }}>{Math.round(selectedLayer.transform?.rotation || 0)}°</span>
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                    <input
                      ...
                      onChange={e => updateLayer(selectedLayer.id, { transform: { ...selectedLayer.transform, rotation: parseFloat(e.target.value) || 0 } })}
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
                  <button
                    onClick={() => updateLayer(selectedLayer.id, { transform: { ...selectedLayer.transform, flipX: !selectedLayer.transform.flipX } })}
                    title="Flip Horizontal"
                    style={{ padding: '8px', backgroundColor: selectedLayer.transform.flipX ? 'rgba(255,255,255,0.15)' : 'rgba(255,255,255,0.05)', border: `1px solid ${BORDER}`, borderRadius: '6px', color: 'white', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', fontSize: '12px', transition: 'background-color 0.2s' }}
                    onMouseEnter={e => e.currentTarget.style.backgroundColor = selectedLayer.transform.flipX ? 'rgba(255,255,255,0.2)' : 'rgba(255,255,255,0.1)'}
                    onMouseLeave={e => e.currentTarget.style.backgroundColor = selectedLayer.transform.flipX ? 'rgba(255,255,255,0.15)' : 'rgba(255,255,255,0.05)'}
                  >
                    <FlipHorizontal style={{ width: '16px', height: '16px' }} /> Flip H
                  </button>
                  <button
                    onClick={() => updateLayer(selectedLayer.id, { transform: { ...selectedLayer.transform, flipY: !selectedLayer.transform.flipY } })}
                    title="Flip Vertical"
                    style={{ padding: '8px', backgroundColor: selectedLayer.transform.flipY ? 'rgba(255,255,255,0.15)' : 'rgba(255,255,255,0.05)', border: `1px solid ${BORDER}`, borderRadius: '6px', color: 'white', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', fontSize: '12px', transition: 'background-color 0.2s' }}
                    onMouseEnter={e => e.currentTarget.style.backgroundColor = selectedLayer.transform.flipY ? 'rgba(255,255,255,0.2)' : 'rgba(255,255,255,0.1)'}
                    onMouseLeave={e => e.currentTarget.style.backgroundColor = selectedLayer.transform.flipY ? 'rgba(255,255,255,0.15)' : 'rgba(255,255,255,0.05)'}
                  >
                    <FlipVertical style={{ width: '16px', height: '16px' }} /> Flip V
                  </button>
                </div>
              </div>
              ...
              {(selectedLayer.type === 'video' || selectedLayer.type === 'image') && (
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
                  <button
                    onClick={() => {
                      const layerRatio = selectedLayer.transform.width / selectedLayer.transform.height;
                      const canvasRatio = canvas.width / canvas.height;
                      let newWidth, newHeight;
                      if (layerRatio > canvasRatio) {
                          newWidth = canvas.width;
                          newHeight = canvas.width / layerRatio;
                      } else {
                          newHeight = canvas.height;
                          newWidth = canvas.height * layerRatio;
                      }
                    ...
                    style={{ padding: '8px', backgroundColor: 'rgba(255,255,255,0.05)', border: `1px solid ${BORDER}`, borderRadius: '6px', color: 'white', fontSize: '12px', cursor: 'pointer', transition: 'background-color 0.2s' }}
                  ...
                  <button
                    onClick={() => {
                      updateLayer(selectedLayer.id, {
                          transform: {
                              ...selectedLayer.transform,
                              x: (canvas.width - (selectedLayer.transform.width || 0)) / 2,
                              y: (canvas.height - (selectedLayer.transform.height || 0)) / 2
                          }
                      });
                    }}
                    style={{ padding: '8px', backgroundColor: 'rgba(255,255,255,0.05)', border: `1px solid ${BORDER}`, borderRadius: '6px', color: 'white', fontSize: '12px', cursor: 'pointer', transition: 'background-color 0.2s' }}
              ...
              {selectedLayer.type === 'text' && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                  ...
                  <div>
                    <label style={{ fontSize: '12px', color: TEXT_MUTED, marginBottom: '8px', display: 'block' }}>Format</label>
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '8px' }}>
                      <button
                        onClick={() => updateLayer(selectedLayer.id, { isBold: !selectedLayer.isBold })}
                        style={{ padding: '8px', backgroundColor: selectedLayer.isBold ? 'rgba(255,255,255,0.15)' : 'rgba(255,255,255,0.05)', border: `1px solid ${BORDER}`, borderRadius: '6px', color: 'white', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'background-color 0.2s' }}
                      >
                        <Bold style={{ width: '16px', height: '16px' }} />
                      </button>
                      <button
                        onClick={() => updateLayer(selectedLayer.id, { isItalic: !selectedLayer.isItalic })}
                        style={{ padding: '8px', backgroundColor: selectedLayer.isItalic ? 'rgba(255,255,255,0.15)' : 'rgba(255,255,255,0.05)', border: `1px solid ${BORDER}`, borderRadius: '6px', color: 'white', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'background-color 0.2s' }}
                      >
                        <Italic style={{ width: '16px', height: '16px' }} />
                      </button>
                      <button
                        onClick={() => updateLayer(selectedLayer.id, { isUnderline: !selectedLayer.isUnderline })}
                        style={{ padding: '8px', backgroundColor: selectedLayer.isUnderline ? 'rgba(255,255,255,0.15)' : 'rgba(255,255,255,0.05)', border: `1px solid ${BORDER}`, borderRadius: '6px', color: 'white', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'background-color 0.2s' }}
                      >
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '8px' }}>
                        <button
                          ...
                          style={{ padding: '8px', backgroundColor: selectedLayer.align === 'left' ? 'rgba(255,255,255,0.15)' : 'rgba(255,255,255,0.05)', border: `1px solid ${BORDER}`, borderRadius: '6px', color: 'white', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'background-color 0.2s' }}
                          title="Align Left"
                        >
                        ...
                        <button
                          ...
                          style={{ padding: '8px', backgroundColor: (!selectedLayer.align || selectedLayer.align === 'center') ? 'rgba(255,255,255,0.15)' : 'rgba(255,255,255,0.05)', border: `1px solid ${BORDER}`, borderRadius: '6px', color: 'white', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'background-color 0.2s' }}
                          title="Align Center"
                        >
                        ...
                        <button
                          ...
                          style={{ padding: '8px', backgroundColor: selectedLayer.align === 'right' ? 'rgba(255,255,255,0.15)' : 'rgba(255,255,255,0.05)', border: `1px solid ${BORDER}`, borderRadius: '6px', color: 'white', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'background-color 0.2s' }}
                          title="Align Right"
                        >
                      ...
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '8px' }}>
                        <button
                          onClick={() => updateLayer(selectedLayer.id, { verticalAlign: 'top' })}
                          style={{ padding: '8px', backgroundColor: selectedLayer.verticalAlign === 'top' ? 'rgba(255,255,255,0.15)' : 'rgba(255,255,255,0.05)', border: `1px solid ${BORDER}`, borderRadius: '6px', color: 'white', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'background-color 0.2s' }}
                          title="Align Top"
                        >
                          <AlignStartVertical style={{ width: '16px', height: '16px' }} />
                        </button>
                        <button
                          onClick={() => updateLayer(selectedLayer.id, { verticalAlign: 'middle' })}
                          style={{ padding: '8px', backgroundColor: (!selectedLayer.verticalAlign || selectedLayer.verticalAlign === 'middle') ? 'rgba(255,255,255,0.15)' : 'rgba(255,255,255,0.05)', border: `1px solid ${BORDER}`, borderRadius: '6px', color: 'white', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'background-color 0.2s' }}
                          title="Align Middle"
                        >
                          <AlignCenterVertical style={{ width: '16px', height: '16px' }} />
                        </button>
                        <button
                          onClick={() => updateLayer(selectedLayer.id, { verticalAlign: 'bottom' })}
                          style={{ padding: '8px', backgroundColor: selectedLayer.verticalAlign === 'bottom' ? 'rgba(255,255,255,0.15)' : 'rgba(255,255,255,0.05)', border: `1px solid ${BORDER}`, borderRadius: '6px', color: 'white', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'background-color 0.2s' }}
                          title="Align Bottom"
                        >
                          <AlignEndVertical style={{ width: '16px', height: '16px' }} />
                        </button>
                      </div>
                    </div>
                  </div>
