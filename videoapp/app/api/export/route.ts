    ...
    return new Promise<NextResponse>((resolve) => {
      ...
      ffmpeg.on("close", (code) => {
        if (code === 0) {
          ...
          try {
             fs.writeFileSync(path.join(renderDir, "ffmpeg_error.log"), "Command:\nffmpeg " + args.join(" ") + "\n\nLog:\n" + errorLog);
          } catch(e) {}
          resolve(NextResponse.json({ error: "FFmpeg render failed. See temp/renders/ffmpeg_error.log", log: errorLog }, { status: 500 }));
        }
      });
    });
